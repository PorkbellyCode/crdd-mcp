...
const QUIZ_INSTRUCTIONS = [
  ...
  "1. 일반적인 프로그래밍 상식만 묻는 질문은 만들지 마세요. 다만 매우 중요한 개념이라면, material에 담긴 이 프로젝트의 실제 코드를 예시로 들어 그 개념을 이해했는지 확인하는 질문은 허용됩니다 — 이 경우에도 질문의 근거와 예시는 항상 이 프로젝트의 구조/코드/변경사항이어야 합니다.",
  ...
  questions: [
    {
      ...
      codeExcerpt:
        "질문의 근거가 되는 실제 코드 발췌 (파일 경로와 줄 범위를 표기하고, 마크다운 코드 블록으로 제공 — 사용자가 파일을 열지 않아도 문제를 풀 수 있는 최소 범위)",
